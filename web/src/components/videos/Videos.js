...
function Videos() {
  ...
  const loadItems=(page) => {
    ...
    videos.map((video)=>{ video.id=video.id+Math.random();currentVideos.push(video)});
    console.log("page",page,currentVideos);
    setVideos(currentVideos);
    if(videos.length>200) setHasMoreVideos(false);
